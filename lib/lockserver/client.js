...
function _lock(names,opts,handler) {

	var
		self = this,
		_names = (names instanceof Array) ? names : [names];
	...
	_debug("[client] Locking resources "+_names.join(', ')+" (got it #"+id+")...");

	self.requests[id] = { names: _names, opts: opts, handler: handler };
	...
	return self._clientWaitConnection(function(err){
		...
		_debug("[client] Sending lock request #"+id+" on resources "+_names.join(', '));
		return self._command("lock",{names: _names, id: id, timeout: opts.timeout, iflock: opts.iflock });
	});

}

function _ifLock(names,opts,lockedHandler,handler) {
	...
	return self.lock(names,opts,handler);
...
function _unlock(id,handler) {
	...
	_debug("[client] Sending unlock request #"+id+" on resources '"+req.names.join(', ')+"'");
	...
	if ( m.command == "answer" ) {
		if ( m.to == "lock" ) {
			...
			if ( m.islocked && req.opts.iflock ) {
				_debug("[client] Server told that resources '"+req.names.join(', ')+"' (request #"+m.id+") are locked. waiting...");
