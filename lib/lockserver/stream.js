...
function Stream(format,con) {
	...
	self.notifyDrain = notifyDrain;

	// If has a connection, listen her events

	if ( con ) {
		con.on('drain',function(){    self.notifyDrain();     });
...
function sendMessage(msg,handler) {

	if ( this.con == null )
		return handler(new Error("For sending messages throw the stream you need to supply the socket connection on the constructor()"),null);
...
//	console.log("buf: ",buf);
	...
	if ( this.con.write(buf) && handler )
		handler(null,true);
	else if ( handler )
		this.waitingDrain.push(handler);

}

// If everything was written, notify about drai

function notifyDrain() {

	while ( this.waitingDrain.length > 0 ) {
		var handler = this.waitingDrain.shift();
		handler(err,true);
	}
